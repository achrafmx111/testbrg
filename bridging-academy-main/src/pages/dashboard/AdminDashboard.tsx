...
const AdminDashboard = () => {
    ...
    const fetchInterviewRequests = async () => {
        ...
        try {
            const { data, error } = await (supabase as any)
                ...
                .select('*, application:applications(name, email, course_name), employer:profiles(email)')
                .order('created_at', { ascending: false });

            if (error) {
                // If table doesn't exist yet, ignore error silently for now or log
                console.warn("Interview requests table might not exist yet:", error);
                return;
            }
    ...
    return (
        <div className={`space-y-6 ${isArabic ? "rtl" : "ltr"}`} dir={isArabic ? "rtl" : "ltr"}>
            ...
            {/* Phase 5: Interview Requests Section */}
                ...
                <Card className="border-l-4 border-l-purple-500 shadow-md">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-lg font-bold flex items-center gap-2">
                            ...
                            <Badge className="ml-2 bg-purple-100 text-purple-700 hover:bg-purple-100">{interviewRequests.filter(r => r.status === 'pending').length} Pending</Badge>
                        </CardTitle>
                        <CardDescription>
                                    ...
                                    {interviewRequests.map((req: any) => (
                                        <TableRow key={req.id}>
                                            ...
                                            <TableCell className="max-w-[200px]">
                                                <p className="truncate text-xs text-muted-foreground" title={req.message}>{req.message || "No message"}</p>
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            navigate(`/dashboard/candidate/${app.id}`);
                                                        }}
                                                    >
