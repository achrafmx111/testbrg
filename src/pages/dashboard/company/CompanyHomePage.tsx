...
const funnelData = [
    ...
    { stage: "Screening", count: 280, color: "hsl(var(--secondary))" },
    { stage: "Interview", count: 120, color: "hsl(var(--accent))" },
    { stage: "Offer", count: 45, color: "hsl(var(--primary) / 0.65)" },
    { stage: "Hired", count: 22, color: "hsl(var(--secondary) / 0.65)" },
];
...
export default function CompanyHomePage() {
    ...
    const statCards = [
        ...
        { label: "New Applicants", value: stats.newApplicants, change: "+12.5%", icon: UserPlus, color: "text-secondary", bg: "bg-secondary/10", border: "border-secondary/20 shadow-secondary/10" },
        ...
        { label: "Hired", value: stats.hired, change: "Target: 25", icon: CheckCircle2, color: "text-primary", bg: "bg-primary/10", border: "border-primary/20 shadow-primary/10" },
    ];
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10">
            ...
            <motion.div
                ...
                className="relative group overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-card via-card to-primary/15 p-8 md:p-14 text-foreground shadow-2xl border border-border/60"
            >
                {/* Dynamic Background */}
                <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-primary/10 to-transparent blur-3xl -z-0"></div>
                <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-secondary/10 rounded-full blur-[100px] -z-0"></div>

                <div className="relative z-10 grid md:grid-cols-2 gap-8 items-center">
                    <div className="space-y-6">
                        <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-xs font-semibold backdrop-blur-xl border border-primary/20 text-foreground tracking-wide uppercase">
                        ...
                        <div className="space-y-4">
                            ...
                            <p className="text-muted-foreground max-w-md text-lg leading-relaxed">
                                Your hiring pipeline is performing <span className="text-foreground font-medium">15% better</span> than last month. Ready to close some deals?
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-4 pt-4">
                            <Link to="/company/jobs">
                                <Button size="lg" className="h-14 px-8 rounded-2xl shadow-xl shadow-primary/20 transition-all hover:scale-105 active:scale-95 group">
                            ...
                            <Link to="/company/talent-pool">
                                <Button size="lg" variant="outline" className="h-14 px-8 rounded-2xl border-border/60 bg-card/60 backdrop-blur-md transition-all">
                    ...
                    <div className="hidden lg:block bg-card/70 backdrop-blur-2xl border border-border/60 rounded-3xl p-6 shadow-2xl h-64">
                        <div className="flex items-center justify-between mb-4">
                            <p className="text-sm font-semibold text-muted-foreground">Application Velocity</p>
                            <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20">+24%</Badge>
                        </div>
                ...
                <AnimatePresence>
                    {statCards.map((stat, index) => (
                        <motion.div
                            ...
                            whileHover={{ y: -8 }}
                        >
                            <Card className={`group border ${stat.border} bg-card/80 backdrop-blur-xl shadow-lg transition-all duration-500 overflow-hidden rounded-3xl`}>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-baseline gap-2">
                                                    <h3 className="text-4xl font-extrabold text-foreground tracking-tighter">{stat.value}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Hiring Funnel Chart */}
                <Card className="lg:col-span-2 rounded-[2rem] border-border/60 bg-card/80 backdrop-blur-md shadow-xl overflow-hidden">
                    <CardHeader className="flex flex-row items-center justify-between p-8">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            className="rounded-xl border-secondary/20 text-secondary hover:bg-secondary/5"
                            onClick={() => toast.info("Showing current quarter data")}
                        >
                            Current Quarter
                        </Button>
                    </CardHeader>
                ...
                <div className="space-y-8">
                    <Card className="rounded-[2.5rem] border-slate-200 dark:border-navy/40 shadow-xl">
                        <CardHeader className="p-8 pb-4">
                            <CardTitle className="text-xl font-bold flex items-center gap-2">
                                <TrendingUp className="h-5 w-5 text-primary" /> Recent Pulse
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 pt-0">
                            <div className="space-y-8">
                                {activities.map((activity, index) => (
                                    ...
                                    >
                                        <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${activity.type === 'system' ? 'bg-primary/10 text-primary' : 'bg-primary/5 text-primary'}`}>
                            ...
                            <Link to="/company/pipeline">
                                <Button variant="secondary" className="w-full mt-10 h-12 rounded-2xl bg-muted hover:bg-muted/80 font-bold transition-all text-foreground">
                    ...
                    <Card className="bg-primary rounded-[2.5rem] text-primary-foreground p-8 shadow-2xl shadow-primary/20 overflow-hidden relative group">
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <Button
                                className="w-full bg-white text-foreground hover:bg-slate-100 rounded-2xl font-bold"
                                onClick={() => {
                                    navigator.clipboard.writeText("https://bridging.academy/invite/c/123");
