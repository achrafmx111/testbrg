...
const Apply = () => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('courses')
          .select('slug, title, title_de, category_label_de, duration_de, level_de, language, price, is_premium, extra_fields');
        ...
        if (error) {
          ...
          setCourses(data.map(c => ({
            ...
            title: c.title,
            titleDe: c.title_de,
            categoryLabelDe: c.category_label_de,
            durationDe: c.duration_de,
            levelDe: c.level_de,
            language: c.language,
            price: c.price,
            isPremium: c.is_premium,
            extraFields: c.extra_fields
          })));
          setFetchError(null);
        }
      } catch (err: any) {
  ...
  // Reset extra fields when course changes
  useEffect(() => {
    setExtraFieldValues({});
  }, [selectedCourse]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Note: File uploads moved to Talent Pool page
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          country_city: applicationData.country_city,
          course_slug: applicationData.course_slug,
          course_name: applicationData.course_name,
          learning_mode: applicationData.learning_mode,
          message: applicationData.message,
          extra_fields: applicationData.extra_fields,
          status: 'pending',
          user_id: user?.id || null,
          cv_path: applicationData.cv_path,
          passport_path: applicationData.passport_path,
          cert_path: applicationData.cert_path
        });

      if (dbError) {
        console.error("Database Error:", dbError);
        throw new Error(isGerman ? "Fehler beim Speichern der Bewerbung." : "Error saving application.");
      }
                ...
                <Card className="border-2 border-primary/20 bg-primary/5 mb-6">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div>
                        <Badge className="mb-2">{selectedCourseData?.categoryLabelDe || 'Course'}</Badge>
                        <h3 className="text-xl font-heading font-bold text-foreground">
                          {selectedCourseData?.titleDe || selectedCourseData?.title}
                        </h3>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{selectedCourseData?.durationDe || 'TBA'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <BookOpen className="h-4 w-4 text-primary" />
                        <span>{selectedCourseData?.levelDe || 'All Levels'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Globe className="h-4 w-4 text-primary" />
                        <span>{selectedCourseData?.language ? selectedCourseData.language.split('/')[0].trim() : 'DE/EN'}</span>
                      </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                          ...
                          <Input
                            ...
                            value={
                              loadingCourses
                                ...
                                : (selectedCourseData
                                  ? (isGerman ? selectedCourseData.titleDe : selectedCourseData.title)
                                  : searchParams.get("course") || "")
                            }
                            className="h-12 bg-muted/50 font-medium text-foreground cursor-not-allowed opacity-90"
                          />
                          {!loadingCourses && !selectedCourseData && (
                            <p className="text-xs text-destructive mt-1">
                              {isGerman ? "Kurs-ID nicht gefunden." : "Course ID not found."}
                            </p>
                          )}
                        </div>
                      ) : (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          key={profile?.id} // Force re-render when profile loads
                        />
                      </div>
                    ...
                    <div className="p-4 rounded-lg bg-secondary/10 border border-secondary/20">
                      <p className="text-sm text-foreground flex items-center gap-2">
                        ...
                        {isGerman
                          ? "Möchten Sie im Talent Pool aufgenommen werden? Nach der Anmeldung können Sie Ihre Dokumente dort zentral hochladen."
                          : "Want to join our Talent Pool? After applying, you can upload your verification documents there."}
                      </p>
                      <Button asChild variant="link" className="p-0 h-auto text-primary text-sm mt-1">
                        <Link to="/talent-pool">
                          {isGerman ? "Zum Talent Pool →" : "Go to Talent Pool →"}
                        </Link>
                      </Button>
                    </div>
